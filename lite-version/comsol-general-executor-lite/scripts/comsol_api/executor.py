...
from .couplings import build_couplings
from .derived_values import build_derived_values
from .exports import build_exports
from .plots import build_numerical_results, build_plots, build_result_tables
from .profiles import apply_profile
from .registry import (
    api_type_for,
    registries,
)
from .solvers import build_solvers, run_solvers
from .utils import (
...
from .wave_optics import build_wave_optics_helpers
...
class OfficialPlanExecutor:
    ...
    def __init__(self, model, plan: dict[str, Any]):
        ...
        self.plan = plan
        self.comsol_version = str(plan.get("comsol_version", "6.4"))
        self.registries = registries(self.comsol_version)
    ...
    def _build_geometry(self, component_tag: str, geom_tag: str, geometry: dict[str, Any]) -> None:
        ...
        for item in geometry.get("features", []):
            api_type = api_type_for(
                self.registries["geometry_features"], item, "Geometry feature"
            )
            tag = str(item["tag"])
            geom.create(tag, api_type)
            feature = geom.feature(tag)
            label_if_present(feature, item)
            set_properties(feature, item.get("properties"))
            set_indexed_properties(feature, item.get("indexed_properties"))
        geom.run()
    ...
    def _build_selections(
        ...
        for selection in selections:
            ...
            if "dimension" in selection:
                ...
                selection_obj.set("entitydim", JInt(int(selection["dimension"])))
            ...
            set_indexed_properties(selection_obj, selection.get("indexed_properties"))
    ...
    def _build_materials(self, component_tag: str, materials: list[dict[str, Any]]) -> None:
        ...
        for material in materials:
            ...
            for group in material.get("property_groups", []):
                ...
                set_properties(props, group.get("properties"))
                set_indexed_properties(props, group.get("indexed_properties"))
    ...
    def _build_physics(
        ...
        for physics in physics_entries:
            physics = apply_profile(
                {"comsol_version": self.comsol_version, **physics}
            )
            api_type = api_type_for(
                self.registries["physics_interfaces"], physics, "Physics interface"
            )
            tag = str(physics["tag"])
            ...
            set_properties(interface, physics.get("properties"))
            set_indexed_properties(interface, physics.get("indexed_properties"))
            set_property_groups(interface, physics.get("property_groups"))
            if "field" in physics:
                field = physics["field"]
    ...
    def _create_physics_feature(self, interface, feature_spec: dict[str, Any]) -> None:
        feature_spec = apply_profile(
            {"comsol_version": self.comsol_version, **feature_spec}
        )
        tag = str(feature_spec["tag"])
        if not feature_spec.get("existing", False):
            api_type = api_type_for(
                self.registries["physics_features"],
                feature_spec,
                "Physics feature",
            )
            from jpype.types import JInt

            dimension = JInt(int(feature_spec.get("dimension", 1)))
        ...
        set_indexed_properties(feature, feature_spec.get("indexed_properties"))
        set_property_groups(feature, feature_spec.get("property_groups"))
        apply_named_selections(feature, feature_spec.get("named_selections"))
    ...
    def _build_mesh(self, component_tag: str, mesh_tag: str, mesh: dict[str, Any]) -> None:
        ...
        for feature in mesh.get("features", []) or []:
            self._create_mesh_feature(mesh_obj, feature)
        if mesh.get("run", True):
            mesh_obj.run()

    def _create_mesh_feature(self, parent, feature: dict[str, Any]) -> None:
        tag = str(feature["tag"])
        parent.create(tag, str(feature["api_type"]))
        feature_obj = parent.feature(tag)
        label_if_present(feature_obj, feature)
        if "selection_dimension" in feature:
            from jpype.types import JInt

            feature_obj.selection().geom(
                str(feature.get("geometry_tag", "geom1")),
                JInt(int(feature["selection_dimension"])),
            )
        ...
        for child in feature.get("features", []) or []:
            self._create_mesh_feature(feature_obj, child)
    ...
    def build_studies(self) -> None:
        for study in self.plan.get("studies", []):
            ...
            for feature in study.get("features", []):
                api_type = api_type_for(
                    self.registries["study_types"], feature, "Study feature"
                )
                feature_tag = str(feature["tag"])
                self.java.study(tag).create(feature_tag, api_type)
                feature_obj = self.java.study(tag).feature(feature_tag)
                set_properties(feature_obj, feature.get("properties"))
                set_indexed_properties(feature_obj, feature.get("indexed_properties"))

    def build_variables(self) -> None:
        build_variables(self.java, self.plan)

    def build_couplings(self) -> None:
        build_couplings(self.java, self.plan)

    def build_wave_optics_helpers(self) -> None:
        build_wave_optics_helpers(self.java, self.plan)

    def build_solvers(self) -> None:
        build_solvers(self.java, self.plan)

    def run_solvers(self) -> None:
        run_solvers(self.java, self.plan)

    def build_result_tables(self) -> None:
        build_result_tables(self.java, self.plan)

    def build_derived_values(self) -> None:
        build_derived_values(self.java, self.plan)

    def build_numerical_results(self) -> None:
        build_numerical_results(self.java, self.plan)

    def build_exports(self) -> None:
        build_exports(self.java, self.plan)
    ...
    def build_results(self) -> None:
        build_plots(self.java, self.plan)
