...
For repeated structures, use the generic `$for_each` and `$repeat` manifest
macros instead of duplicating feature objects. For repeated literature
simulations, do not resend the whole paper to the runner. Send only a compact
JSON manifest or a small diff that includes the reviewed COMSOL interface and
