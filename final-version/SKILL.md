---
name: comsol-general-executor-final
...
10. For A/B evaluation, freeze allowed inputs before execution and use `experiment_audit.py`; do not count hidden memory as evidence.
...
Read [references/evaluation_protocol.md](references/evaluation_protocol.md) before
claiming a completion score. A score of 95 or more requires every critical gate
to pass; averaging cannot cancel a failed physical or reproducibility gate.
...
python scripts/experiment_audit.py --spec experiment_spec.json
