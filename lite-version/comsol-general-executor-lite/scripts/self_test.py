...
def validate_frontmatter() -> list[str]:
    ...
    header = text[4:end]
    if "name: comsol-general-executor-lite" not in header:
...
def main() -> int:
    ...
    compact_task = ROOT / "assets" / "templates" / "lite-diffusion-1d.json"
    compact_output = temp_root / "lite-diffusion-manifest.json"
    compile_result = subprocess.run(
        [
            sys.executable,
            str(SCRIPTS / "compact_task.py"),
            "--task",
            str(compact_task),
            "--output",
            str(compact_output),
    ...
    checks.append(
        {
            "name": "compact_task_compile",
            "status": "pass" if compile_result.returncode == 0 else "fail",
            "output": (compile_result.stdout + compile_result.stderr)[-1000:],
        }
    )
