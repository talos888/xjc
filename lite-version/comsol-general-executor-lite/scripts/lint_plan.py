...
from comsol_api.registry import registries
...
def validate_profile(
    spec: dict[str, Any], label: str, available_profiles: dict[str, Any]
) -> list[str]:
    name = spec.get("profile")
    if name and name not in available_profiles:
...
def lint_plan(plan: dict[str, Any]) -> list[str]:
    issues: list[str] = []
    version = str(plan.get("comsol_version", "6.4"))
    try:
        available_profiles = profiles(version)
        available_registries = registries(version)
    except (FileNotFoundError, ValueError, KeyError) as exc:
        return [f"COMSOL API cache unavailable or invalid: {exc}"]
    ...
    for component in plan.get("components", []) or []:
        ...
        for feature in geometry_features:
            issues.extend(
                has_registered_or_explicit(
                    feature,
                    available_registries["geometry_features"],
                    "Geometry",
                )
            )
        ...
        for physics in component.get("physics", []) or []:
            issues.extend(validate_profile(physics, "Physics", available_profiles))
            issues.extend(
                has_registered_or_explicit(
                    physics,
                    available_registries["physics_interfaces"],
                    "Physics",
                )
            )
            ...
            for feature in physics.get("features", []) or []:
                issues.extend(
                    validate_profile(feature, "Physics feature", available_profiles)
                )
                issues.extend(
                    has_registered_or_explicit(
                        feature,
                        available_registries["physics_features"],
                        "Physics feature",
                    )
                )
    ...
    for study in plan.get("studies", []) or []:
        for feature in study.get("features", []) or []:
            issues.extend(
                has_registered_or_explicit(
                    feature,
                    available_registries["study_types"],
                    "Study feature",
                )
            )
